...
### The eIDAS qualified-status attestation is covered only one epoch later

`Anchor.tsa_qualified`, `eutl_ref` and `qualified_checked_at` record whether the TSA was an
eIDAS qualified service **at the moment of stamping**. They cannot go into the `epoch_hash` of
the epoch they describe, and the exclusion is forced rather than chosen: the verdict is only
computable once the token comes back, because the TSA's signing certificate is inside it — and
`epoch_hash` is the *input* being stamped. Writing it back would invalidate the timestamp that
was just obtained. Same constraint that keeps `tsa_token` out, and this project has already
shipped that bug once.

`Ledger.attach_anchor()` therefore queues the verdict, and the next `seal_epoch()` writes it
into epoch N+1's hashed body, where the next anchor's timestamp covers it. Altering any field
of it changes epoch N+1's hash and its timestamp stops verifying. `Ledger.verify()` separately
checks that the verdict points at the previous epoch's actual hash and at the token actually
stored there — without that, what the hash protects is only a stretch of text that cannot be
edited, not a statement about this chain.

Two gaps survive that design.

**The last verdict is always unbound.** Nothing has hashed it yet, so it can still be changed
without trace. This is structural, the same shape as "the most recent records are not yet
sealed", and the answer is the same: seal and anchor again.
`Ledger.unbound_anchor_count()` reports it, because a deployment that assumes every verdict is
protected will be wrong about exactly one of them.

**There is no official archive of past national trusted lists.** A list downloaded today
answers today's question. When an auditor's recomputation disagrees with a bound verdict, the
binding proves the recorder wrote that verdict at that time — it does not prove the verdict was
correct. The tool reports the disagreement and stops; it cannot distinguish "the status was
withdrawn in the meantime" from "the verdict was wrong when made". Deployments that need this
settled must archive the signed list XML alongside the verdict; `eutl_snapshot_sha256` is bound
into the chain so that archive can be reconciled against what was actually consulted.
